...
function parseval_crop(imp,cutoff_type::String="ibit",cutoff::Real=24,efract::Real=0.985)
	...
	for rawi in imp
		...
		thresamp=AbstractFloat.(thresamp) #This will make it so the partial sums extermely accurate and not affected by roundoff error
		...
		thresamp=abs2.(thresamp) #Gets the power signals
		...
		increment_mode=false # flag that tells it to just step values 1 at a time
...
function biir(signal,bands::Integer=1,h_frequency::Real=20000.0,l_frequency::Real=20.0)
	#=
	biir_sig - the linked list used to store the results
	gen_bands - store the genereated center frequencies and filters
	band_filter - this stores the actual octave band filters from generateband. it makes it easier to follow
	center - these store the center frequency of the filters
	fil_l - the length the array to tell you how many elements your filter has somit can be indexed over
	sig_f - the filtered signal
	n_l - the length of the filted signal
	new_name - the name of the curve generated by the filtered signal
	=#
